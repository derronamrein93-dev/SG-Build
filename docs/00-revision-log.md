...
| --- | --- | --- | --- |
| 1 | Phase 0 gate too rigid | **Accepted.** Gate is now *one committed design partner OR two retailers willing to run structured discovery/usability testing*. Eight operational discovery facts added and modeled as real `location` fields. | [09 §2](09-build-plan.md#2-phase-0--discovery), [05 §3](05-data-model.md#4-location) |
...
| 3 | `store` → `organization`/`location` | **Accepted, and it propagated widely.** Tenancy is now organization → location → user, with the customer relationship org-owned and location-scoped by grant (revision 3 below). UI still says "store." | [05 §1–§5](05-data-model.md#1-tenancy-organization--location) |
...
| 12 | Report should anticipate customer identity | **Accepted.** Report is a record (`report` + `report_view`) with `report_version`, `template_version`, content snapshot, and `generated_at` / `emailed_at` / `printed_at` / view log. Access resolves customer → session → report_version. Tokens stored hashed. | [04 §2b](04-fit-report.md#2b-the-report-is-a-record-not-a-file), [05 §17](05-data-model.md#22-report-and-report_view) |
...
| 14 | Follow-up UI premature | **Accepted.** Full data model retained (`follow_up_reason`, `follow_up_due_at`, `follow_up_status`), UI cut to a due list and a Done button. | [05 §18](05-data-model.md#23-follow_up), [02 §10](02-ux-spec.md) |
| 15 | "What changed" is important — model it | **Accepted.** `assessment_delta` is now a stored entity computed at completion from immutable feature snapshots, with per-feature from/to/direction/magnitude, a `material_change` flag and a stored narrative. Queryable, not a render-time diff. | [05 §14](05-data-model.md#19-assessment_delta--what-changed-since-last-visit) |
| 16 | Catalog is the wrong abstraction | **Accepted.** Three layers: `product_model` (global knowledge) → `product_variant` (sellable) → `location_inventory` (assortment). Quantity optional, assortment required. | [05 §15](05-data-model.md#20-catalog-three-layers), [06 §3](06-tech-and-data-strategy.md#three-layers-never-one-table) |
...
| 23 | Device identity model needed | **Accepted.** `device`, `device_installation`, `device_health_event`. Every scan records device, firmware, calibration, hardware revision and derivation algorithm. | [05 §19](05-data-model.md#24-device-device_installation-device_health_event) |
| 24 | Raw data must be immutable | **Accepted.** Raw captures immutable with checksum; `scan_derivation` is versioned and re-runnable so algorithm v2 reprocesses old captures and every past customer benefits. | [05 §11–§12](05-data-model.md#16-scan--raw-capture-hardware-immutable) |
...
## Revision 3 — customer identity architecture

One further instruction, applied in full. It supersedes the revision-2 decision
to make `customer.location_id` the owning column.

| Directive | Implementation |
| --- | --- |
| Retailer-scoped relationships, optionally linked to a global identity | `organization_customer` (the retailer relationship, numbered per org) + `person_identity` (global, optional) — [05 §6–§7](05-data-model.md#6-person_identity--global-optional-deliberately-almost-empty) |
| `person_identity_id` nullable | Nullable, and **null is the normal state**. Day 1 runs entirely without a global identity; nothing to migrate when MyStrideID arrives |
| Records stay organization/location scoped | `organization_customer` is org-owned; location scoping is an authorization grant (`location_customer_access`), not an FK — grantable, revocable, auditable |
| Identity resolution must not imply visibility | `identity_resolution` is a five-state layer (`unlinked` → `candidate_match` → `verified` → `linked` → `revoked`) that authorizes **nothing**. `verified` answers "same person" and grants no access |
| Identity, consent, authorization are separate | Three independent mechanisms, stated as the load-bearing principle of the data model — [05 §2](05-data-model.md#2-identity-consent-authorization--three-independent-controls) |
| Not an ordinary link table | Named and modeled as an identity-resolution layer with evidence, proposer, verifier, revocation and an append-only transition log. No automatic matching on hash equality |
| Portable vs retailer-owned | Explicit classification — [05 §12](05-data-model.md#12-portable-vs-retailer-owned-data). Portability is a **projection**, never a transfer; retailer-owned data has no consent path to portability |

### One addition this revision makes on its own

**Per-organization keys for contact hashes.** With a single global HMAC key, two
retailers' rows for the same person produce *identical* `phone_lookup_hash`
values — meaning the platform could correlate customers across retailers as a
silent side effect of the schema, before anyone wrote a line of resolution logic.
That would defeat the separation of identity from authorization at the storage
layer.

So: `organization_customer.phone_lookup_hash` is keyed **per organization**, and
`person_identity.identity_lookup_hash` uses a **separate global key** used only
inside deliberate resolution. Cross-retailer correlation becomes impossible by
accident and possible only through the consented path.
[05 §10](05-data-model.md#10-contact-identity--keyed-hashes-scoped-per-organization)

### What this changes about the roadmap

Nothing on Day 1 except roughly twenty minutes of migration. The identity tables
are created and left unused. What it buys is that the MyStrideID commercial
decision — cross-retailer network or not — never has to be made under schema
pressure.
...
**On mark 3 (organization/location).** Adopted, then refined twice. Revision 2
put `customer.location_id` as the owning column to match the system map.
Revision 3 replaces that with `organization_customer` owned by the organization
plus `location_customer_access` as an explicit grant — the same isolation
guarantee through a better mechanism, and the one that leaves room for optional
cross-retailer identity. `user` remains location-owned with a join for staff who
cover two doors.

## Still open, needing a decision from you

1. ~~**Open question 13 — cross-retailer customer identity.**~~ **Closed
   architecturally in revision 3**, and deliberately left open commercially. The
   schema supports it without committing to it. What still needs answering after
   the pilot: whether retailers will participate and on what terms, exactly what
   sits in a portable Fit Profile, and whether a consumer MyStrideID account
   earns its own product surface.
